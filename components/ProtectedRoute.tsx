...
export default function ProtectedRoute({
  ...
  // Timeout backstop (should rarely trigger after auth fixes)
  useEffect(() => {
    if (loading) {
      const timeoutId = setTimeout(() => {
        setTimedOut(true);
      }, 10000); // 10 seconds - backstop for network issues
  ...
  // Show timeout error (backstop for network failures)
  if (timedOut && loading) {
    return (
